...
#  linear chi-squared fit line
def chi_squared_fit(x, y, sig):
    sig   = sig**2
    norm  = (1./ sig).sum()
    x_    = (x / sig).sum() / norm
    y_    = (y / sig).sum() / norm
    slope = ((x - x_) * y / sig).sum()/((x - x_)*x/sig).sum()
    intercept     = y_ - slope * x_
    sig_slope     = 1./ ( (x - x_) * x / sig).sum()
    sig_intercept = sig_slope * (x * x / sig).sum() / norm
    return slope, intercept

slope, intercept = chi_squared_fit(temperature, pressure, error);
